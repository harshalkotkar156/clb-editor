...
router.post('/execute', async (req, res) => {
  try {
    ...
    // save to MongoDB immediately with status: queued
    ...
    // return jobId immediately — client will poll with this
...
// GET /api/status/:jobId — client polls this
router.get('/status/:jobId', async (req, res) => {
  try {
    ...
    // if still queued or running, return status only
    if (execution.status === 'queued' || execution.status === 'running') {
      return res.json({ jobId, status: execution.status });
    }

    // if done (completed or failed), return full result
    return res.json({
      jobId,
...
// GET /api/history — last 20 executions
router.get('/history', async (req, res) => {
  try {
    const history = await Execution.find()
      .sort({ createdAt: -1 })
      .limit(20)
      .select('-code');   // don't send code back in history list
